        ...
        <>
          ...
          {currentTask !== null ? (
            <div className="w-full flex items-center justify-center ">
              <div className="flex flex-col w-8/12  p-5 rounded-md space-y-3 m-4 ">
                <div className=" w-full flex items-center justify-start space-x-2 grayscale ">
                  <p className=" text-2xl font-bold">{currentTask.title}</p>
                ...
                <div className=" flex items-center justify-start space-x-2 grayscale ">
                  <Badge variant="outline">
                    <Route className="mr-2 text-xs w-4" /> {currentTask.status}
                  </Badge>
                  <p className="text-sm">
                    <span className="hover:font-semibold cursor-pointer ">
                      {currentTask.createdBy.name}
                    </span>{" "}
                    openned this issue on{" "}
                    {currentTask.createdBy.time.toLocaleString()}{" "}
                  </p>
                  <p className="text-sm">
                    · {currentTask.comments.length} comments
                  </p>
                </div>
                <div className="  rounded-md space-y-1 border relative">
                  <div className="space-x-1 flex items-center justify-between border-b bg-slate-50 p-2 h-8">
                    <div className="flex items-center justify-start space-x-1 h-3">
                      <p className="text-sm space-x-2 font-semibold ml-1">
                        {currentTask.createdBy.name} ·{" "}
                      </p>
                      <Badge>Author</Badge>
                    </div>
                    ...
                    <div
                      className={
                        isEditing.index === index
                          ? "  rounded-md space-y-1 border bg-slate-50 shadow-md relative transition-all"
                          : "  rounded-md space-y-1 border relative"
                      }
                    >
                      <div
                        className={
                          isEditing.index === index
                            ? "space-x-1 flex items-center justify-between border-b bg-slate-200 p-2 h-8 transition-all"
                            : "space-x-1 flex items-center justify-between border-b bg-slate-50 p-2 h-8"
                        }
                      >
                        <div className="flex items-center justify-start space-x-1 h-3">
                          <p className="text-sm space-x-2 font-semibold ml-1">
                          ...
                          {comment.owner === currentTask.createdBy.name ? (
                            <Badge className="h-4 text-xs">Author</Badge>
                          ) : (
                            ""
                          )}
                        </div>
                      ...
                      >
                        {isEditing.index === index ? (
                          <Form {...form2}>
                            <form
                              onSubmit={form2.handleSubmit(onSubmitNewComment)}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription>
                              You can <span>@mention</span> other users related
                              to the current task.
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
